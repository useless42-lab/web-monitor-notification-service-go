...
func (checkMinecraftServerJob CheckMinecraftServerJob) Run() {
	var deviceType int = 8
	minecraftServerList := models.GetAllActiveMinecraftServerList()
	for _, item := range minecraftServerList {
		successPercent := models.GetMonitorMinecraftServerSuccessPercent(item.FailedWaitTimes, item.Id)
		if successPercent.Percent == 0 {
			...
			if time.Now().After(notificationLog.CreatedAt.Add(+time.Hour * time.Duration(24))) {
				...
				for _, item2 := range targetNotification {
					...
					go PushNotification(data, checkMinecraftServerJob.Bot)
